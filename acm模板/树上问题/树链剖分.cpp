/*
-
现在要在树上的任意一条路径(或者链)上做操作：
对于一颗完全二叉树：遍历从每个节点开始的重链，总和时间复杂度为o(n)，单次最坏o(logn)；
对于一颗普通的树：单次最坏o(n)，总和最坏o(n^2)。
-
树链剖分：
对树上节点重新编号为id[x]，代表重链连续的dfs序，并使用线段树维护所有新编号；
链上操作拆分为logn条重链，每条链操作复杂度logn；
顺便能完成子树操作。
时间复杂度o(n(logn)^2)。
*/
